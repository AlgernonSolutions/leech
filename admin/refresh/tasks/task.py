...
def _get_activity(domain_name, activity_name, activity_version):
    ...
    task_list = task_config.retrieve('defaultTaskList', {})
    running_timeout = task_config.retrieve('defaultTaskStartToCloseTimeout', _defaults['task_run_timeout'])
    total_timeout = task_config.retrieve('defaultTaskScheduleToCloseTimeout', _defaults['task_total_timeout'])
    waiting_timeout = task_config.retrieve('defaultTaskScheduleToStartTimeout', _defaults['task_waiting_timeout'])
    heartbeat_timeout = task_config.retrieve('defaultTaskHeartbeatTimeout', _defaults['task_heart_timeout'])
    return {
        'task_name': activity_type['name'],
        'task_description': activity_info.retrieve('description', None),
        'task_config': {
            'task_list': task_list.retrieve('name', None),
            'time_outs': {
...
def _create_activity(domain_name, task_config, version='1'):
    ...
    register_args = {
        ...
        'defaultTaskStartToCloseTimeout': time_outs.retrieve('running', _defaults['task_run_timeout']),
        'defaultTaskHeartbeatTimeout': time_outs.retrieve('heartbeat', _defaults['task_heart_timeout']),
        'defaultTaskScheduleToStartTimeout': time_outs.retrieve('waiting', _defaults['task_waiting_timeout']),
        'defaultTaskScheduleToCloseTimeout': time_outs.retrieve('total', _defaults['task_total_timeout']),
    }
    if config.retrieve('task_list', None):
