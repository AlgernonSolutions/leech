...
def _build_change_data_group(**kwargs):
    ...
    change_actions = source_args['change_actions']
    ...
    enrich_operation = activities[names['enrich']]
    enrichment = json.loads(enrich_operation.results, cls=AlgDecoder)
    for remote_change in remote_actions:
        task_args.add_arguments({'source': enrichment})
        task_args.add_arguments({'source': {'remote_change': remote_change}})
        signature = Signature.for_activity(fn_identifier, 'generate_remote_id_change_data', **kwargs)
